      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="(cartInfo,index) in cartInfoList" :key="cartInfo.id">
          <li class="cart-list-con1" >
            <input type="checkbox" name="chk_list" :checked="cartInfo.isChecked" @click="changeChecked(cartInfo)"/>
          </li>
          ...
          <li class="cart-list-con4">
            <span class="price">{{cartInfo.skuPrice||0}}</span>
          </li>
...
export default {
  ...
  //删除购物车数据的某一个产品
     deleteCart : throttle(async function(cartInfo) {    
     try{
  ...
  //修改购物车选中状态
 async changeChecked(cartInfo){
       console.log(cartInfo);
       if(cartInfo.isChecked==1){
         cartInfo.isChecked=0
       }else if (cartInfo.isChecked==0){
         cartInfo.isChecked=1
       }
       try{
          await  this.$store.dispatch('getUpdateCart',{skuId:cartInfo.skuId,isChecked:cartInfo.isChecked});
           this.getData();
       }catch(error){
         alert(error.message)
       }
 ...
 async updateAllChecked(event){
    //console.log(event.target.checked)的值为true 或者FALSE
    try{
...
//全选复选框
isAllCheck(){
  // every 返回true或者 false
  //遍历数组里面的元素只要有一个不为真，就是false  
